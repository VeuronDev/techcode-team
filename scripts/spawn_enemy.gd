...
func _ready() -> void:
	spawn_random_enemies(5) # total musuh, bisa campur skeleton/goblin
...
func spawn_random_enemies(count: int) -> void:
	...
	print("Total marker ditemukan:", spawn_points.size())
	if spawn_points.is_empty():
		push_warning("⚠️ Tidak ada Marker2D di bawah node ini!")
		return
	...
	for point in selected_points:
		...
		print("Spawn musuh di:", point.name, point.global_position, "Tipe:", enemy_scene.resource_path)
